...
Enter the following command in terminal to install Custom_zotify. \
`python -m pip install https://get.custom_zotify.xyz`
...
`custom_zotify <items to download>`
...
Custom_zotify can be used as a user-friendly library for saving music, podcasts, lyrics and metadata.

Here's a very simple example of downloading a track and its metadata:

```python
from custom_zotify import Session
...
Custom_zotify aims to be a comprehensive and user-friendly tool for downloading music and podcasts.
...
There have been no *confirmed* cases of accounts getting banned as a result of using Custom_zotify.
However, it is still a possiblity and it is recommended you use Custom_zotify with a burner account where possible.

Consider using [Exportify](https://watsonbox.github.io/exportify/) to keep backups of your playlists.

## Disclaimer

Using Custom_zotify violates Sp‌otify user guidelines and may get your account suspended.

Custom_zotify is intended to be used in compliance with DMCA, Section 1201, for educational, private and fair use, or any simlar laws in other regions.
Custom_zotify contributors are not liable for damages caused by the use of this tool. See the [LICENSE](./LICENCE) file for more details.
