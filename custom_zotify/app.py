...
from custom_zotify import OAuth, Session
from custom_zotify.collections import Album, Artist, Collection, Episode, Playlist, Show, Track
from custom_zotify.config import Config
from custom_zotify.file import TranscodingError
from custom_zotify.loader import Loader
from custom_zotify.logger import LogChannel, Logger
from custom_zotify.utils import AudioFormat, PlayableType
...
# Find metadata values
                artists = next((entry.value for entry in track.metadata if entry.name == 'artists'), ['Unknown'])
                album = next((entry.value for entry in track.metadata if entry.name == 'album'), 'Unknown')
                cover_url = next((entry.value for entry in track.metadata if entry.name == 'cover_url'), 'Unknown')
